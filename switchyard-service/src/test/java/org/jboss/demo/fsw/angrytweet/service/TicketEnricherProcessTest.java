...
public class TicketEnricherProcessTest extends JbpmJUnitBaseTestCase {
    ...
    @Test
    public void testProcessCustomerNotFound() {
        createRuntimeManager("process/TicketEnricherProcess.bpmn");
        RuntimeEngine runtimeEngine = getRuntimeEngine();
        KieSession session = runtimeEngine.getKieSession();
        MockSwitchyardServiceTaskHandler mockWih = new MockSwitchyardServiceTaskHandler();
        ...
        GetAccountInfoResponse getAccountInfoResponse = new GetAccountInfoResponse();
        getAccountInfoResponse.setResult(false);
        ...
        Assert.assertTrue(mockWih.verifyServiceNotInvoked("StoreService"));
        
        Assert.assertTrue(mockWih.verifyServiceNotInvoked("ChannelOutService"));
        
        Assert.assertTrue(mockWih.verifyServiceNotInvoked("RouterService"));

        Assert.assertTrue(mockWih.verifyServiceInvoked("ErrorStoreService", 1));
        Assert.assertNotNull(mockWih.getObjectPassedAsParameter("Parameter", "ErrorStoreService", 1));
        Assert.assertTrue(mockWih.getObjectPassedAsParameter("Parameter", "ErrorStoreService", 1) instanceof ProviderServiceTicket);
        Assert.assertEquals(ticket, mockWih.getObjectPassedAsParameter("Parameter", "ErrorStoreService", 1));
        
        Assert.assertTrue(ticket.isError());
        Assert.assertNotNull(ticket.getErrorString());
        Assert.assertEquals("Customer with customer code '" + ticket.getRequester() + "' not found in CRM", ticket.getErrorString());        
    }
